...
//   const floatingRef = useRef(null); // NEW: Reference for floating DOM elements
...
//     return { 
//       x: (x - offset.current.x) / scale.current, 
//       y: (y - offset.current.y) / scale.current 
//     };
//   };
...
//       previewCanvasRef.current.style.transform = transform;
//       previewCanvasRef.current.style.transformOrigin = "0 0";
//     }
//     if (floatingRef.current) {
//       floatingRef.current.style.transform = transform;
//       floatingRef.current.style.transformOrigin = "0 0";
//     }
//   }, []);
...
//   const drawStroke = useCallback((stroke) => { replayStrokes([stroke]); }, [replayStrokes]);
...
//     ctx.fillStyle = "#ffffff"; ctx.fillRect(0, 0, 400, 225);
...
//         ctx.lineWidth = size.current; ctx.lineCap = "round"; ctx.lineJoin = "round";
//         ctx.stroke();
...
//         socket.emit("draw-stroke", { roomId, stroke: { type: tool.current, points: currentStroke.current, color: color.current, size: size.current } });
//         currentStroke.current = [];
//         scheduleThumbnail();
//       } else if (tool.current === "shape") {
//         const preview = previewCanvasRef.current;
//         if (preview) preview.getContext("2d").clearRect(0, 0, preview.width, preview.height);
//         const stroke = { type: "shape", shapeType: shapeType.current, x1: startPos.current.x, y1: startPos.current.y, x2: pos.x, y2: pos.y, color: color.current, size: size.current };
//         drawShapeOnCtx(ctx, stroke);
...
//     const handleMouseDown = (e) => { if (e.button === 1 || e.button === 2) { e.preventDefault(); isPanning.current = true; lastPanPos.current = { x: e.clientX, y: e.clientY }; } };
...
//       }
//     };
//     const handleMouseUp = (e) => { if (e.button === 1 || e.button === 2) isPanning.current = false; };
...
//   return {
//     canvasRef, previewCanvasRef, floatingRef, replayStrokes, drawStroke, generateThumbnail, scale, offset,
